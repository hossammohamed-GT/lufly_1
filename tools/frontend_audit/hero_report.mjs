...
const cards = rows.map((row) => {
  ...
  const band = row.band || 0;
  const copyArea = row.hero === null ? 0 : row.hero - band;
  ...
  <div class="screen" style="height:${scale}px;width:${Math.round((row.width / row.height) * scale)}px">
    <div class="hero" style="height:${px(row.hero)}">
      ${band ? `<div class="band" style="height:${px(band)}"><span>artwork band ${band}px</span></div>
      <div class="copy" style="top:${px(band)}"><span>copy area ${copyArea}px</span></div>` : ''}
    </div>
    <div class="needs" style="bottom:${row.hero === null ? 0 : px(row.hero)}"><span>fills the open screen ${row.hero}px</span></div>
  </div>
  <p class="nums">hero <b>${row.hero}px</b>${band ? ` · band ${band}px · photo shows ${Math.round(row.cropFraction * 100)}% of its width` : ` · max ${row.max}px`} · ${failed ? row.issues.join('; ') : 'ok'}</p>
</figure>`;
}).join('\n');
...
<style>
  ...
  .band { position: absolute; inset-inline: 0; top: 0; background: #1d2b33; }
  .band span, .copy span { position: absolute; inset-inline-start: 4px; top: 3px; font-size: 9.5px; color: #cfd8dd; }
  .copy { position: absolute; inset-inline: 0; bottom: 0; border-top: 1px dashed #7b8794; }
  .copy span { color: #7b8794; }
...
<p class="lede">Every frame is a simulated device viewport. The teal block is the height the hero ends up with - the whole open screen - and the red dashed line marks its bottom, so it must sit exactly on the device's own bottom edge. On a portrait phone the dark strip at the top of the hero is the artwork band and the dashed line under it is where the copy starts: the band is measured from the copy, so the photo never gets cropped into a close-up. Generated by <code>tools/frontend_audit/hero_report.mjs</code>, which reads the same numbers the checks assert on.</p>
...
<ul>
  ...
  <li><b>The phone hero is stacked.</b> A full-height hero on a phone squeezes a landscape photo into a ~0.5 aspect box, which shows about a quarter of the frame - that is what made the artwork feel cramped. The artwork now keeps a wide band at the top (height measured from the copy, capped at half the screen) and the copy lives underneath, so the hero fills the screen <em>and</em> the photo keeps a comfortable crop.</li>
